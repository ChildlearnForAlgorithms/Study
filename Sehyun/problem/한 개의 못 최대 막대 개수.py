...
for _ in range(n):
    ...
    lst.append((a, "f"))  # 리스트에 시작점과 first를 나타낼 'f'를 튜플로 묶어 삽입
    lst.append((b, "l"))  # 리스트에 끝점과 last를 나타낼 'l'를 튜플로 묶어 삽입
...
리스트에 왼쪽 끝 점과 오른쪽 끝 점을 넣어줄 때 시작점인지 끝점인지 구분할 수 있도록 각각 'f'와 'ㅣ'을 넣어주었다.
